...
(function ($) {
    ...
    $.fn.columnFilter = function (options) {
        ...
        function fnCreateInput(oTable, regex, smart, bIsNumber, iFilterLength, iMaxLenght) {
            ...
            var input = $('<input type="text" placeholder="'+ label +'" class="' + search_init + sCSSClass + '" value="' + inputvalue + '" rel="' + i + '"/>');
            if (iMaxLenght != undefined && iMaxLenght != -1) {
                input.attr('maxlength', iMaxLenght);
            }
            th.html(input);
            if (bIsNumber)
                th.wrapInner('<div class="ui fluid input filter_column filter_number" />');
            else
                th.wrapInner('<div class="ui fluid input filter_column filter_text" />');
        ...
        function fnCreateRangeInput(oTable) {
            ...
            th.wrapInner('<div class="ui fluid input filter_column filter_number_range form-control" />');
            var index = i;
        ...
        function fnCreateColumnSelect(oTable, aData, iColumn, nTh, sLabel, bRegex, oSelected, bMultiselect) {
            ...
            var select = $(r + '</select>');
            nTh.html(select);
            nTh.wrapInner('<div class="ui fluid filter_column filter_select" />');
        ...
        function fnCreateDropdown(aData) {
            ...
            var select = $(r + '</ul></div>');
            th.html(select);
            th.wrapInner('<div class="ui fluid search dropdown selection filterColumn filter_select" />');
            select.find('li').click(function () {
                oTable.fnFilter($(this).data('value'), index);
            });
        }

        function fnCreateSemanticDropdown(aData,aID) {
            var index = i;
            var r = '<select id="'+ aID +'"><option value=""></option>';

            for (var j = 0; j < aData.length; j++) {
                r += '<option value="' + aData[j] + '">' + aData[j] + '</option>';
            }
            r += '</select><i class="dropdown icon"></i><input class="search" tabindex="0"><div class="text"></div><div class="menu transition hidden" tabindex="-1">';
            for (var j = 0; j < aData.length; j++) {
                r += '<div class="item" data-value="' + aData[j] + '">' + aData[j] + '</div>';
            }
            var select = $(r + '</div>');
            th.html(select);
            th.wrapInner('<div class="ui fluid search dropdown selection filterColumn filter_select" />');
            ...
            select2.onchange = function () {
                oTable.fnFilter(select2.value, index);
            };
        }

        function fnCreateResetButton() {
            var r = '<button class="ui icon button"><i class="refresh icon"></i>Réinit.</button>';
            var button = $(r);
            th.html(button);
            var nbColumn = oTable[0].rows[0].cells.length;
            var footer = oTable[0].rows[(oTable[0].rows.length)-1].cells;
            button[0].onclick = function () {
                for(var j = 0;j<nbColumn;j++){
                    var input = footer[j].getElementsByTagName("input")[0];
                    var items = footer[j].querySelectorAll('.item');
                    var text = footer[j].querySelectorAll('div .text');
                    if(text.length > 0){
                        text[0].innerHTML = "";
                    }
                    ...
                    $.each(items, function (index, value) {
                        ...
                        $(value)[0].classList.remove('selected');
                    });
                    oTable.fnFilter("", j);
                }
            };
        ...
        function fnCreateCheckbox(oTable, aData) {
            ...
            for (j = 0; j < iLen; j++) {
                ...
                var checkbox = $(r);
                th.html(checkbox);
                th.wrapInner('<div class="ui fluid filter_column filter_checkbox" />');
                ...
                if (aoColumn != null) {
                    ...
                    switch (aoColumn.type) {
                        ...
                        case "reset-button":
                            fnCreateResetButton();
                            break;
                        case "semantic-dropdown":
                            fnCreateSemanticDropdown(aoColumn.values, aoColumn.id);
                            break;
                        case "dropdown":
